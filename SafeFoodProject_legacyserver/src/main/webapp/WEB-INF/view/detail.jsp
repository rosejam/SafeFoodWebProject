...
<script type="text/javascript">
	...
	function drawChart() {
		...
		var data = google.visualization.arrayToDataTable([
				[ 'Task', 'Hours per Day' ], [ '칼로리', 1 ], [ '탄수화물', 2 ],
				[ '지방', 3 ], [ '콜레스트롤', 4 ], [ '포화지방산', 5 ], [ '당류', 6 ],
				[ '당류', 7 ], [ '단백질', 8 ] ]);
...
<script>
	$(function() {
		var responseMessage = "<c:out value="${almsg}" />";
		if (responseMessage != "") {
...
<body>
	...
	<div class="container">
		<div class="row">
			<div id='container'>
				Name :
				<h1 id='name'>${b.name }</h1>
				Maker :
				<h1 id='maker'>${b.maker}</h1>
				<br> <br>
				<hr>
				<div class='container'>
					<div id='image1'>
						<img src="resources/${b.img}" id='first_image' />
					</div>
					<div id='infoname'>
						1회 제공량
						<h1>${b.supportpereat}</h1>
					</div>
					<div id='infoname'>
						칼로리
						<h1>${b.calory}</h1>
		...
		<p>
			<button class="btn btn-info" id ="addfood">
				<a href='addfood.food?code=${b.code }'>추가</a>
			</button>
			<!-- <button class="btn btn-info"><a href='#'>찜</a></button></p> -->
	</div>
